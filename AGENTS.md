...
- 真实环境验证取代 mock 冒充；数据库以真实 PostgreSQL 验证，UI 以真实浏览器走查并把结论写入任务。临时截图不得提交，稳定视觉回归基线必须单独获批。
...
- 所有自然语言注释使用中文；标识符、协议名、库名、JSDoc 标签和工具指令可保留原文。
- 每个手写 JS、TS、TSX 文件以一句中文 `@fileoverview` 开头；每个函数、方法和 React 组件以一句中文 JSDoc 说明存在目的或关键约束。
- CSS、SQL、Shell、YAML 与支持注释的手写配置使用中文文件说明。JSON、Markdown、生成文件、锁文件和第三方代码豁免。
- 注释只保留职责、约束、副作用、不变量或非显然原因，不记录修改历史、排障过程、方案争论和可由代码直接读出的细节。
- 禁止无信息量参数说明、正反例堆砌、逐行翻译、模板化长注释、注释掉的代码和生产 `console.log`。
