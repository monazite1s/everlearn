...
- 响应式断点三档：`md`（48rem/768px，移动/桌面）、`xl`（80rem/1280px）与 `2xl`（96rem/1536px，编辑器常驻右栏档），在主题入口 CSS 以 `--breakpoint-*` 定义；组件内自适应优先容器查询（`@container`）。顶栏高度以 `--header-height: 3rem` token 记录，业务代码禁止写死 48px 或 `3rem` 字面量。
