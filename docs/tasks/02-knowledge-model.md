...
- 状态：已完成（2026-08-18，中断恢复后由主 agent 实现 + 复验走查；首个 agent 完成契约客户端 `convertInboxItem` 后因配额与模型流停滞两次中断）。
- 依赖：KB-09、KB-08W、KB-06W。
- 必读：`docs/01-design/pages/knowledge-base.md`、`docs/01-design/design-system.md`、`docs/02-architecture/api-and-events.md`。
- 目标：Inbox 可选择知识库与父节点并幂等转换，成功后进入新文档。
- 改动：`inbox-api.ts` 新增 `convertInboxItem`（严格校验、Idempotency-Key、201 `DocumentDetail`）；新增转换对话框（目标知识库 Select〔第一页〕→ 父级 Select〔知识库根 + 目标库根级文档一级〕→ 标题 Input 默认取记录内容前 80 字符）；行操作「转换为文档」桌面渲染；页面接线 offline。
- 设计要点：目标/父级候选用「派生 loading + 异步回调 setState」模式（无 effect 内同步 setState）；幂等键首次提交生成、失败重试复用、成功/取消丢弃、409 换新键；成功后列表重读（记录消失以服务端为准）并跳转目标知识库概览（文档正文页面属 ED-05，未伪造路由）；404（记录已转换/目标失效）重读列表后自动关闭对话框；父级候选仅已加载根级一级（按需加载架构一致，未展开子树需先到目标库展开，对话框内说明）。
- 验证结果：组件测试 2 文件 `20 passed`（新增 7：默认标题+载荷断言+导航、已知失败保留选择、不确定失败同键重试、409 换新键、404 重读关框、移动端无入口、知识库加载失败提示）；Web typecheck、聚焦 ESLint、Prettier、注释（215 文件 1725 条）、文件限制与生产构建通过。
- 真实证据：Playwright（Chrome headless，真实 API）5 步通过——真实记录打开对话框（默认标题=记录内容）、选择目标库「Everlearn 开发记录」与知识库根、提交后跳转 `/knowledge/:id` 且文档树显示新文档、Inbox 记录消失、390px 无转换入口；截图当次会话人工检查。
- 风险：父级候选仅根级一级（多级父级需先到目标库展开，ED-05 编辑器上线后可评估树形选择器）；知识库候选仅第一页（>20 个库时需分页选择，登记升级项）；本任务未经独立 code-reviewer 第二视角（子 agent 两次基础设施中断后主 agent 按既有评审范式自查实现，模式复刻自已评审的 trash 恢复对话框与 move 对话框——生产加固阶段可补一次独立评审）。
