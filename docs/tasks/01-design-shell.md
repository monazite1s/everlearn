...
- 验证：前期执行组件测试、Web Typecheck、生产构建和人工移动视口检查；Playwright、axe 与截图比较延后到核心流程稳定阶段。

### UI-04 完成证据

- 改动：为六个一级路由声明移动能力；≤768px 隐藏桌面侧栏并提供焦点受控导航抽屉；移动端新建操作改为桌面创作说明，保留搜索和运行入口。
- 验证：移动壳组件测试 2 项通过；按文件 ESLint、Stylelint、全仓 Typecheck 与生产构建退出 0。
- 证据：`renders mobile reading controls` 覆盖导航抽屉、返回焦点和桌面创作说明；Next 生产构建生成六个一级页面。
- 风险：真实移动浏览器视觉与 axe 检查按前期测试策略延后；未实现移动编辑、离线缓存或 PWA。
...
- 改动：以类型安全 Mock 建立最近文档、知识库、快速记录与跨模块运行摘要；覆盖正常、首次使用、加载、局部失败和离线状态，移动端隐藏快速记录。
- 验证：首页 6 个聚焦组件测试、按文件 ESLint/Stylelint、Web Typecheck 与生产构建退出 0。
- 证据：组件测试分别验证知识库主层级、无任务不渲染运行区、局部重试、三个加载区域和离线写入禁用；生产构建生成首页与六个一级路由。
- 风险：当前数据与写入均为静态边界；业务 API、真实重试和 Inbox 写入由后续任务实现，浏览器视觉回归按前期测试策略延后。

## 检查点

完成后运行聚焦组件测试、Web Typecheck、生产构建和 `pnpm check`；人工确认桌面/移动布局以及默认主题没有聊天或 AI 渐变风格。axe 与 Playwright 延后到核心流程稳定阶段。
