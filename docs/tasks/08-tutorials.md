...
7. 修复器不处理字符串内裸双引号：正文含 `git commit -m "..."` 类未转义英文双引号时字符串提前闭合，提取降级或失败。升级条件：该形态在真实失败样本中显著出现时，为修复器增加「字符串内裸 `"` 转义」策略。
8. 空简报落 11 字符占位文档：无新条目的简报仍创建 11 字符占位文档，「无新条目不落文档」规则未实现。升级条件：占位文档形成列表噪音或产品确认跳过规则时，改为空结果直接跳过落库。
9. 同订阅多份同名简报无序号：同一订阅多次生成同名简报文档，标题无序号或日期区分。升级条件：用户反馈同名混淆或需要回溯历史简报时，为标题追加日期或序号。
