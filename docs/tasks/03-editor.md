...
## UI 布局重构（2026-08-18 用户反馈返工）

用户指出三类问题：UI 严重错位（元素位置与常识不符）、文档切换整页硬切导致骨架屏闪烁、布局缺少截断/极值/flex 放缩纪律。受理后先取证再返工——对抗性审计（超长标题夹具 + 多视口扫描 + 点击命中 + 切换过渡帧采样）坐实全部三类；此前走查为假阴性（功能性 DOM 断言无法暴露布局质量）。

### 取证结论（程序化数据）

- w1440 整页横滚 32px：修订面板条目越出视口右缘（right=1456）。
- w1000 整页横滚 108px：树列全部链接越界（right=1076）。
- 树行链接被编辑区段落元素覆盖，点击失效（Playwright 报 intercepts pointer events）。

### 根因（三层，全部修复）

1. 无列定义 grid 轨道按内容 max-content 计宽：树 section/ul 与修订 ol 均为裸 `grid`，truncate 的 nowrap 使 max-content 等于整条 CJK 标题（实测 836px），轨道撑爆 256px 列后溢出内容 bleed 至编辑列下方（劫持点击）并整页横滚。修复：内容链全部加 `grid-cols-1` / `grid-cols-[minmax(0,1fr)]`。
2. 三栏中列 `minmax(40rem,1fr)` 硬底：256（壳）+256（树）+640（中列最小）+320（面板）=1472 超出视口，右栏必被推出。修复：中列 `minmax(0,1fr)` + 面板常驻断点上移 2xl/96rem。
3. 树在叶子路由组件内：`[docId]` 变化整页 remount，树重取加骨架屏闪烁。修复：新增 `documents/layout.tsx` 承载树列（usePathname 提取 activeDocumentId），切换文档树不重挂不重取。

### 返工中连带发现并修复

- 769–1279px 区带壳侧栏加树后编辑列仅 249px，低于工具栏最小宽 286px 必然溢出——树列改 ≥1280px 显示（规格区带修订，壳侧栏与面包屑承担窄幅导航）。
- JS 断点 `48.0625/96.0625em` 偏移与 CSS md/2xl 边界像素分歧（恰 1536px 时 CSS 预留面板列而 JS 不渲染）——面板网格列改由 JS wide 状态驱动，与渲染分支同源。

### ui-designer 位置评审（10 条）处置

| #   | 发现                               | 处置                                                                               |
| --- | ---------------------------------- | ---------------------------------------------------------------------------------- |
| 1   | 双面包屑同词不同标                 | 中列首项改为知识库真实名称（页面层取 KB 名下传），顶栏保持模块定位；规格补分工条款 |
| 2   | 树当前态与 hover 同色              | 当前行加主色脊线（知识脊线落地）+ 标题加粗                                         |
| 3   | 2xl 常驻面板无折叠入口（规格违背） | 标题行右侧统一开合按钮（aria-pressed）                                             |
| 4   | 面板按钮混入格式化工具栏           | 移至标题行右侧与保存徽标成组（TitleActions）                                       |
| 5   | 骨架屏与最终结构不符               | 四段结构（面包屑/标题/工具栏/正文）+ max-w-prose 容器                              |
| 6   | 1280/1440 中列仅 448-608px         | 采纳：面板常驻断点上移 96rem；规格断点三档制（md/xl/2xl）修订                      |
| 7   | 移动端 TOC 常驻加提示卡过重        | TOC 默认折叠；提示降为一行 muted 文案并改措辞                                      |
| 8   | 「文档面板/属性」开发者词汇        | 统一「文档信息」，页签「历史/信息」                                                |
| 9   | 树行零信息图标加 24px/层缩进       | 移除 FileText 图标，缩进降为 12px/层                                               |
| 10  | 行菜单「打开文档」冗余             | 删除，菜单自「新建子文档」起                                                       |

### 验证证据（返工后）

- 组件 204/204；全仓 `pnpm check` exit 0。
- 对抗性扫描：1600/1536/1440/1280/1100/1024/900/800/769/768/390 十一档视口，超长标题加深树夹具下零页级横滚、零越界、零未截断（修复前 1440/1000/769 三档失败）。
- 浏览器验收 10 项通过：六视口干净、树链接可点击且切换后树不重挂（节点数不变、标题正确、无树骨架帧）、2xl 面板折叠/展开、页签「历史/信息」、1280 按钮开 Sheet、移动端 TOC 默认折叠可定位、一行提示；截图目视核对（1600 四列/脊线高亮/真名面包屑、移动 TOC）通过。
- 断点边界：恰 768px 只读、恰 1536px Sheet 档，与规格 769/1537 边界一致。

### 规格修订记录

layout-and-navigation.md：响应式区带四段重写（>1536 三栏 / 1280–1535 左栏+Sheet / 769–1279 左栏暂藏 / ≤768 只读）、断点三档制（md/xl/2xl）、面包屑分工条款；design-system.md 断点条款同步；主题入口补 `--breakpoint-2xl: 96rem`。
