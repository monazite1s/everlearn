...
- `/tutorials/:id/compose` 返回会话快照（分页消息、待处理提案与确认卡、大纲与章节状态）；`POST /tutorials/:id/compose/messages` 以幂等键同步发送消息并返回 200；`POST /tutorials/:id/compose/proposals/:proposalId/accept|reject` 决议提案，重复决议返回首次结果；章节差异接受沿用 `/generations/:id/accept`；compose 流式更新订阅既有 `/workflow-runs/:id/events`。
