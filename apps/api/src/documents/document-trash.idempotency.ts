/** @fileoverview 定义文档恢复幂等键的锁定、请求指纹与首次响应存取。 */
...
export function createRestoreHash(context: RestoreContext): string {
  return createHash('sha256')
    .update(`${RESTORE_OPERATION}\0${context.id}\0${context.version}`)
    .digest('hex');
}

/** 用于串行化并发调用使用的所有者范围幂等键。 */
export async function lockRestoreIdempotencyKey(
  transaction: Transaction<DatabaseSchema>,
  context: RestoreContext,
): Promise<void> {
  const { sql } = await import('kysely');
  const lockKey = `${context.ownerId}:${RESTORE_OPERATION}:${context.idempotencyKey}`;
  await sql`
...
/** 用于返回已存响应或拒绝其他请求复用该键。 */
export async function readRestoreIdempotentResponse(
  transaction: Transaction<DatabaseSchema>,
  context: RestoreContext,
  requestHash: string,
...
/** 用于在同一事务中保存首次成功公开响应。 */
export async function storeRestoreIdempotentResponse(
  transaction: Transaction<DatabaseSchema>,
  context: RestoreContext,
  requestHash: string,
