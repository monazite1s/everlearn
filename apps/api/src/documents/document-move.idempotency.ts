/** @fileoverview 定义文档移动幂等键的锁定、请求指纹与首次响应存取。 */
...
export function createMoveHash(context: MoveContext): string {
  ...
  return createHash('sha256').update(`${MOVE_OPERATION}\0${fingerprint}`).digest('hex');
}

/** 用于串行化并发调用使用的所有者范围幂等键。 */
export async function lockMoveIdempotencyKey(
  transaction: Transaction<DatabaseSchema>,
  context: MoveContext,
): Promise<void> {
  const { sql } = await import('kysely');
  const lockKey = `${context.ownerId}:${MOVE_OPERATION}:${context.idempotencyKey}`;
  await sql`
...
/** 用于返回已存响应或拒绝其他请求复用该键。 */
export async function readMoveIdempotentResponse(
  transaction: Transaction<DatabaseSchema>,
  context: MoveContext,
  requestHash: string,
  ...
  const record = await transaction
    ...
    .where('operation', '=', MOVE_OPERATION)
...
/** 用于在同一事务中保存首次成功公开响应。 */
export async function storeMoveIdempotentResponse(
  transaction: Transaction<DatabaseSchema>,
  context: MoveContext,
  requestHash: string,
